#Written by Sgt DeVo...
#Your friendly neighborhood CySecTech.
#Requirements
$ottenere = {function Get-InjectedThread
{
    <# 
...
do{
#Target
        $RBox = Read-Host -Prompt 'Input host name of remote computer'
#Control
        $Title = "Warning" 
        $Info = "You are connecting from $env:COMPUTERNAME to $RBox "
        $options = [System.Management.Automation.Host.ChoiceDescription[]] @("&Get-Thread", "&Stop-Thread", "&Retry", "&Quit")
        [int]$defaultchoice = 3
        $opt = $host.UI.PromptForChoice($Title , $Info , $Options,$defaultchoice)
            switch($opt){
                0 { Write-Host "Connecting..." -ForegroundColor Green
                        sleep -Seconds 3
                   #Actions
                       Try{$s = New-PSSession $RBox -ErrorAction Stop}
                                        Catch{Write-Host "Target computer cannot be reached..." -ForegroundColor Red -BackgroundColor Black
                                        sleep -Seconds 3
                                        Write-Host "Message: $($_.Exception.Message)" -ForegroundColor DarkCyan -BackgroundColor Black  
                                        sleep -Seconds 3
                                        $opt = 2
                                        Break}
                        Write-Host "Getting threads..." -ForegroundColor Green
                        sleep -Seconds 3
                        Invoke-Command $ottenere -Session $s
                        $g = Invoke-Command -Session $s -ScriptBlock{Get-InjectedThread}
                   #Fin 
                        Invoke-Command -Session $s -ScriptBlock{klist purge} -ErrorAction Inquire
                        Write-Host "Remote credentials have been cleared..." -ForegroundColor Green
                        sleep -Seconds 3
                        Remove-PSSession $RBox
                        Write-Host "Remote connection has been closed..." -ForegroundColor Green
                        sleep -Seconds 3                  
                   #Content Check
                        Out-File .\temp.txt; (Get-Item .\temp.txt).Attributes += 'Hidden'
                        Write-Output $g | Out-File -FilePath .\temp.txt
                            if(-not ((Get-Content .\temp.txt -Raw) -match '\S'))
                                {Add-Content -Path .\temp.txt -Value "No threats were found..."}
                        Get-Content .\temp.txt | Out-File -FilePath .\"$RBox-$(Get-Date -Format "HHmm-ddMMyy").txt"
                        Remove-Item -Path .\temp.txt -Force
                        Write-Host "File has been written to current directory as ComputerName-Time-Date" -ForegroundColor Green
                        sleep -Seconds 5 
                    Break}
                1 { Write-Host "Connecting..." -ForegroundColor Green
                   ...
                   #Actions 
                        Try{$s = New-PSSession $RBox -ErrorAction Stop}
                                        ...
                                        $opt = 2
                                        Break}
                        Write-Host "Stopping threads..." -ForegroundColor Green
                        sleep -Seconds 3
                        Invoke-Command $ottenere -Session $s
                        $k = Invoke-Command -Session $s -ScriptBlock{Get-InjectedThread}
                        Invoke-Command -Session $s -ScriptBlock{$k = Get-InjectedThread}
                        if($k -eq $null){
                            Write-Host "There are no injected threads to stop..." -ForegroundColor Green
                            sleep -Seconds 3
                            Invoke-Command -Session $s -ScriptBlock {klist purge} -ErrorAction Inquire
                            Write-Host "Remote credentials have been cleared..." -ForegroundColor Green
                            sleep -Seconds 3
                            ...
                            sleep -Seconds 3
                            Exit}
                        Invoke-Command $uccidere -Session $s
                        Invoke-Command -Session $s -ScriptBlock{$k.ThreadId | foreach{Stop-Thread -ThreadId $_ }}
                        $k.ThreadId | foreach{Write-Host "Thread $_ has been terminated..." -ForegroundColor Green}
                        sleep -Seconds 3 
                   #Fin               
                        Invoke-Command -Session $s -ScriptBlock {klist purge} -ErrorAction Inquire
                        Write-Host "Remote credentials have been cleared..." -ForegroundColor Green
                        sleep -Seconds 3
                        Remove-PSSession $RBox -ErrorAction Inquire
                        Write-Host "Remote connection has been closed..." -ForegroundColor Green
                        sleep -Seconds 3
                    Break}
                2 { Write-Host "Clearing computer name..." -ForegroundColor Green
                    sleep -Seconds 3}
                3 { Write-Host "Exiting..." -ForegroundColor Green
                    sleep -Seconds 3
                    Exit}
                    }
} while (($opt -ne 2) -xor ($opt -ne 3))
#######END###SCRIPT#######