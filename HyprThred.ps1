...
#Get-ADComputer -Filter 'name -like "2mardiv*"' | Export-Csv .\name1.csv -NoTypeInformation
#^-- this can be used to create the CSV needed to run the functions outlined in this script.
#Requirements
...
#######START#SCRIPT#######
$find = Import-Csv $(Get-ChildItem *.csv)
Write-Host "Initiating scan now..." -ForegroundColor Green
Out-File .\temp.txt; (Get-Item .\temp.txt).Attributes += 'Hidden'
$find.Name | ForEach-Object{$s = New-PSSession $_ -ErrorAction SilentlyContinue ; sleep -Seconds 5;
                         if(-not($s)){
                            Write-Warning "Connection to $_ cannot be established"
                            Write-Host "Message: Verify $_ has the ability to recieve PS-Sessions" -ForegroundColor DarkCyan -BackgroundColor Black
                            Add-Content -Path .\temp.txt "$_ cannot be reached..."}
                         else{
                            Write-Host "Connection to $_ has been established" -ForegroundColor Green
                            sleep -Seconds 3
                            Write-Host "Getting threads from $_ ..." -ForegroundColor Green
                            Invoke-Command $ottenere -Session $s
                            $g = Invoke-Command -Session $s -ScriptBlock{Get-InjectedThread}
                                if($g -eq $null)
                                    {Add-Content -Value "No injected threads found on $_ ..." -Path .\temp.txt}
                                else{Add-Content -Value "Threads found on $_ ..."
                                     Add-Content -Value $g -Path .\temp.txt}
                            Invoke-Command -Session $s -ScriptBlock{klist purge} 
                            Write-Host "Remote credentials have been cleared from $_ ..." -ForegroundColor Green
                            sleep -Seconds 3
                            Remove-PSSession $s
                            Write-Host "Remote connection has been closed..." -ForegroundColor Green
                            sleep -Seconds 3}      
                           }
Get-Content .\temp.txt | Out-File -FilePath .\"ThreadScan-$(Get-Date -Format "HHmm-ddMMyy").txt"
